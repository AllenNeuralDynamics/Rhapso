...
class VoxelVis():
    ...
    def create_voxel_grid(self):
        ...
        self.grid_shape = np.ceil((self.max_coords - self.min_coords) / self.voxel_size).astype(int) + 1
        self.voxel_grid = np.zeros(self.grid_shape, dtype=int) 
        for point in self.just_points:
            voxel_index = np.floor((point - self.min_coords) / self.voxel_size).astype(int)
    ...
    def visualize_voxel_grid(self):
        ...
        ax.scatter(non_empty_voxels[:, 0], non_empty_voxels[:, 1], non_empty_voxels[:, 2],
                   c=counts, cmap='viridis', marker='s')
        ax.set(
            xlabel = 'X Voxel Index',
            ylabel = 'Y Voxel Index',
            zlabel = 'Z Voxel Index'
        )
