...
def execute_job(xml_path, output_path, ray_config_path, channel, input_path, output_s3_path):
    ray_config_dir = Path(ray_config_path).parent
    yml_name = Path(ray_config_path).name
    
    # Clean up any existing cluster
    print("\n=== Clean up any existing cluster ===")
    print("$", " ".join(["ray", "down", yml_name, "-y"]))
    try:
        subprocess.run(["ray", "down", yml_name, "-y"], cwd=ray_config_dir, capture_output=False, text=True)
        print("✅ Cleanup completed (or no existing cluster)")
    except:
        print("ℹ️  No existing cluster to clean up")
    
    # Start the Ray cluster
    start_cluster(yml_name, ray_config_dir)

    try:
        print("\n=== Preparing fusion job ===")
        print(f"Reading resolution from XML: {xml_path}")
        ...
        try:
            print("run ray exec fusion cmd")
            result = subprocess.run(
                ["ray", "exec", yml_name, fusion_cmd],
                cwd=ray_config_dir,
            ...
            cleanup_cluster(yml_name, ray_config_dir)
            raise
        except subprocess.CalledProcessError as e:
...
if __name__ == '__main__':
    ...
    ray_config_path = 'Rhapso/pipelines/ray/aws/config/dev/fusion_cluster_martin.yml'
    channel = 488
    ...
    output_s3_path = 's3://martin-test-bucket/exaSPIM_720164_2025-07-07_17-55-45_rhapso/fusion/fused.zarr'
