...
from . import utils
from .ome_zarr_metadata import write_ome_ngff_metadata
...
def write_plane_to_pyramid(lvl_ds: list, z: int, plane_zy: np.ndarray):
    # level 0
    lvl_ds[0][0, 0, z, :, :] = plane_zy

    # progressively decimate XY for higher levels
    curr = plane_zy
    for level in range(1, len(lvl_ds)):
        # simple 2x decimation (replace with avg pooling if you want anti-aliasing)
        curr = curr[::2, ::2]
        lvl_ds[level][0, 0, z, :, :] = curr

def safe_create_zarr_group(store):
    try:
        return zarr.group(store=store, path="", overwrite=False)
    except ContainsGroupError:
        return zarr.open_group(store=store, path="", mode="r+")

def init_pyramid_writers(
    output_path: str,
    Z: int, Y: int, X: int,
    edge: int,
    out_dtype,
    ...
    """
    One-time OME-NGFF setup. Returns (lvl_ds, Y0, X0) where lvl_ds is a list
    of per-level zarr datasets you can write into as lvl_ds[level][0,0,z,:,:] = slice.
    """
    # corrected XY after radial-correction crop
    Y0 = Y - 2 * edge
    X0 = X - 2 * edge

    stack_name = Path(output_path).name
    parent_path = utils.get_parent_path(output_path)

    # Creating Zarr dataset in s3 or local
    s3 = s3fs.S3FileSystem()
    store = FSStore(parent_path, mode="w", dimension_separator="/", fs=s3)
    root_group = zarr.open_group(store=store, path="", mode="a")
    grp = root_group.create_group(name=stack_name, overwrite=True)

    # NGFF metadata (keep Z, downsample 2x in XY per level)
    write_ome_ngff_metadata(
        group=grp,
        arr_shape=(1, 1, Z, Y0, X0),
        image_name=stack_name,
        n_lvls=n_lvls,
        scale_factors=[1, 2, 2],      # Z, Y, X
        voxel_size=voxel_size,        # [z, y, x]
        channel_names=None,
        channel_colors=None,
        channel_minmax=[(0, 1)],
        channel_startend=[(0, 1)],
        metadata=None,
        final_chunksize=(1, 1, 128, 256, 256),
        origin=[0, 0, 0],
    )

    compressor = Blosc(cname="zstd", clevel=3, shuffle=Blosc.SHUFFLE, blocksize=0)

    # create datasets for all levels
    lvl_ds = []
    Yl, Xl = Y0, X0
    for level in range(n_lvls):
        ds = grp.create_dataset(
            name=str(level),
            shape=(1, 1, Z, Yl, Xl),
            chunks=(1, 1, 128, min(256, Yl), min(256, Xl)),
            dtype=out_dtype,
...
def correct_and_save_tile(
    ...
    corrected_tile = apply_corr_to_zarr_tile(dataset_loc, scale, corner_shift, frac_cutoff, driver=driver)
    end_time = time.time()
    LOGGER.info(
...
def main(
    ...
    """
    zyx_voxel_size = utils.get_voxel_resolution(acquisition_path)
    zarr_path = data_folder
    output_path = results_folder
    
    correct_and_save_tile(
