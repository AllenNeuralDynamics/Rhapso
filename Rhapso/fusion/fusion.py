...
#     # Write slice (moved up so we can zero-fill early)  # <<<
#     output_slice = (
#         slice(0, 1),
#         slice(0, 1),
...
#     kept_src_ids: list[int] = []  # <<< track only tiles that actually contributed

#     for t_id in src_ids:
#         try:  # <<< BEGIN: tolerate partial-OOB tiles
...
#         except Exception:
#             # tile doesn't contribute to this cell (partial out-of-bounds, degenerate crop, etc.)
#             continue  # <<< END

#         overlap_contributions.append(contribution)
#         kept_src_ids.append(t_id)  # <<<

#     # If nothing contributed, explicitly write zeros so cell is not "missing"  # <<<
#     if not overlap_contributions:
#         arr[output_slice] = 0
#         return
...
#         kwargs={
#             "chunk_tile_ids": kept_src_ids,  # <<< use contributing ids, not original src_ids
#             "cell_box": cell_aabb,
#         },
#     )
...
class FusionVolumeSampler(cq.VolumeSampler):
    ...
    # def __iter__(self):
    #     """
    #     Iterates through cells and ALWAYS yields (subject to stride),
    #     even if src_ids is empty.
    #     """
    #     cz, cy, cx = self.cell_size
    ...
    #                     # keep stride behavior
    #                     if (cell_num % self.stride) != self.start:
    #                         continue
    ...
    #                     src_ids = [
    #                         t_id for (t_id, t_aabb) in self.tile_aabbs.items()
    #                         if utils.check_collision(curr_cell, t_aabb)   # output-space AABB hit
    #                     ]
    #                     yield curr_cell, src_ids   # always yield (subject to stride)

    #                     # src_ids: list[int] = [
    #                     #     t_id
    #                     #     for (t_id, _t_aabb) in self.tile_aabbs.items()
    #                     #     if self._check_true_collision(
    #                     #         curr_cell,
