...
ALIGNMENT_BASE = "s3://aind-scratch-data/sean.fite/alignment-test-new-rhapso"
...
def get_pair_matches_one_direction(point_manifest, match_index_df, tp_id: int, setup_a: int, setup_b: int, label: str):
    ...
    for target_label, rows in corr_df.groupby("target_label"):
        ...
        target_view_ids = rows["target_view_id"].to_numpy(dtype=np.int64) if "target_view_id" in rows.columns else np.full(len(rows), -1)
...
def nominal_overlap_to_local_padded_crop(rec: dict, overlap_box_fullres, pad_xy: int):
    ...
    aligned_corners = np.asarray([[ox0, oy0, 0], [ox1, oy0, 0], [ox0, oy1, 0], [ox1, oy1, 0]], dtype=np.float32)
...
def global_scaled_to_a_crop_display_filtered(pts_global_xyz: np.ndarray, rec_a: dict, crop_a_full: dict, scale_a: dict, image_a_crop_shape):
...
def compute_match_qc(match_a_display: np.ndarray, match_b_display: np.ndarray, image_shape_zyx, scale: dict):
    ...
    good_error = qc["median_consistency_residual_scaled"] <= 3.0 and qc["p90_consistency_residual_scaled"] <= 8.0
    good_spread = np.isfinite(qc["grid_occupancy"]) and qc["grid_occupancy"] >= 0.20 and np.isfinite(qc["max_cell_fraction"]) and qc["max_cell_fraction"] <= 0.25
    good_unique = np.isfinite(qc["duplicate_a_fraction"]) and qc["duplicate_a_fraction"] <= 0.05 and np.isfinite(qc["duplicate_b_fraction"]) and qc["duplicate_b_fraction"] <= 0.05
...
def print_match_qc(qc, scale: dict):
    ...
    print(f"  scaled xyz:                 dx={qc['median_dx']:.2f}, dy={qc['median_dy']:.2f}, dz={qc['median_dz']:.2f}")
    print(f"  full-res approx xyz:        dx={qc['median_dx'] * scale['scale_x']:.1f}, dy={qc['median_dy'] * scale['scale_y']:.1f}, dz={qc['median_dz'] * scale['scale_z']:.1f}")
    print(f"  MAD scaled xyz:             dx={qc['mad_dx']:.2f}, dy={qc['mad_dy']:.2f}, dz={qc['mad_dz']:.2f}")
...
def make_pair_ring_segments(a_xy: np.ndarray, b_xy: np.ndarray, min_radius_px: float = 7.0, pad_px: float = 4.0, n_vertices: int = 48):
    if len(a_xy) == 0:
        return []
...
def view_match_pair_slider(vol_a_crop: np.ndarray, pts_a_display: np.ndarray, pts_b_display: np.ndarray, match_a_display: np.ndarray, match_b_display: np.ndarray, qc, setup_a: int, setup_b: int):
    ...
    im = ax.imshow(norm(z0), cmap="gray", origin="upper", aspect=IMAGE_ASPECT)
    a0, b0, seg0, rings0, colors0, _ = visible_matches_for_slice(match_a_display, match_b_display, residuals, z0, LINK_Z_RADIUS_SCALED)
    ...
    ax.legend(loc="upper left", bbox_to_anchor=(1.005, 1.0), borderaxespad=0.0, fontsize=8, frameon=True)
    ax.set_title(f"Match QC after full XML transform stack | red=A setup {setup_a} | blue=B setup {setup_b}", fontsize=11, pad=8)

    info_text = fig.text(0.15, 0.135, "", ha="left", va="center", fontsize=10, family="monospace")
    ...
    def update(val):
        ...
        sc_a_match.set_offsets(a_now[:, :2] if len(a_now) else np.empty((0, 2)))
        sc_b_match.set_offsets(b_now[:, :2] if len(b_now) else np.empty((0, 2)))
...
def dedupe_match_pairs(match_a_local, match_b_local, decimals=3):
    ...
    key_arr = np.concatenate([np.round(match_a_local, decimals=decimals), np.round(match_b_local, decimals=decimals)], axis=1)
...
def prepare_pair_data(rec: dict, overlap_box_fullres, point_manifest):
    ...
    vol_crop = vol[:, crop_scaled["y0"]:crop_scaled["y1"], crop_scaled["x0"]:crop_scaled["x1"]].compute().astype(np.float32)

    print(f"  setup {rec['setup']} crop full x[{crop_full['x0']}:{crop_full['x1']}] y[{crop_full['y0']}:{crop_full['y1']}]")
    print(f"  setup {rec['setup']} crop scaled zyx={vol_crop.shape}")

    print(f"Loading setup {rec['setup']} interest points")
    pts_all = read_interest_points(point_manifest, rec["setup"], TIMEPOINT, MATCH_LABEL)
...
def main():
    ...
    print("Loading point/match manifests...")
    ...
    match_a_display_all = global_scaled_to_a_crop_display_unfiltered(match_a_global_scaled, rec_a, data_a["crop_full"], data_a["scale"])
    match_b_display_all = global_scaled_to_a_crop_display_unfiltered(match_b_global_scaled, rec_a, data_a["crop_full"], data_a["scale"])

    pair_display_mask = display_bounds_mask(match_a_display_all, vol_a_crop.shape) & display_bounds_mask(match_b_display_all, vol_a_crop.shape)
