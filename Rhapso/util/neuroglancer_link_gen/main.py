...
from .hcr_link import generate_hcr_link, generate_multi_zarr_hcr_link
...
def parse_s3_upload_path(json_upload_bucket, parent_directory, json_upload_path=None):
    ...
    if json_upload_path:
        ...
        s3_key = f"{parent_directory}/fused_ng.json"
...
def generate_neuroglancer_link(
    ...
    opacity=0.5,
    blend="default",
    zarr_paths=None,        # 🔹 NEW: optional list of Zarr paths for multi-channel HCR
):
    """
    ...
    :param zarr_paths: Optional list of additional Zarr paths (e.g., fused channels) to
                       include as separate layers in a single HCR NG state.
    """
    # Normalize to a list
    ...
    if zarr_paths:
        print(f"  Extra Zarr Paths ({len(zarr_paths)}):")
        for p in zarr_paths:
            print(f"    - {p}")
    print(f"  VMin: {vmin}")
    print(f"  VMax: {vmax}")
    ...
    # If multiple zarr_paths are given, use the first as representative for parent_directory
    representative_path = zarr_paths[0] if zarr_paths else zarr_path
    s3_bucket, parent_directory = parse_s3_path(representative_path)

    # Determine which bucket to use for the neuroglancer link
    bucket_path = json_upload_bucket if json_upload_bucket else "aind-scratch-data"
    ...
    if chosen_mode == "auto":
        # If user passed multiple paths, that strongly implies multi-channel HCR
        if zarr_paths and len(zarr_paths) > 1:
            chosen_mode = "hcr"
        else:
            chosen_mode = "hcr" if is_hcr_dataset(zarr_path) else "exaspim"

    if chosen_mode == "hcr":
        # ✅ Multi-channel HCR fused Zarrs
        if zarr_paths and len(zarr_paths) > 1:
            print("📊 Using HCR processing for multiple fused channels...")
            generate_multi_zarr_hcr_link(
                s3_paths=zarr_paths,
                vmin=vmin,
                vmax=vmax,
                opacity=1.0,           # HCR spec: full opacity
                blend="additive",      # HCR spec: additive blend
                output_json_path=str(local_output_path),
                dataset_name=parent_directory,
                bucket_path=bucket_path,
            )
        else:
            # Original single HCR behavior
    ...
    output_json_file = local_output_path / "fused_ng.json"
    ...
    if json_upload_bucket:
        try:
            ...
            try:
                ...
                if '/' in upload_key:
                    new_ng_link = (
                        f"https://neuroglancer-demo.appspot.com/#!"
                        f"s3://{upload_bucket}/{'/'.join(upload_key.split('/')[:-1])}/{Path(output_json_file).name}"
                    )
                else:
                    new_ng_link = (
                        f"https://neuroglancer-demo.appspot.com/#!"
                        f"s3://{upload_bucket}/{Path(output_json_file).name}"
                    )
                ...
                if json_upload_path:
                    ...
                    if key_parts[-1].endswith('.json'):
                        new_ng_link = (
                            f"https://neuroglancer-demo.appspot.com/#!"
                            f"s3://{upload_bucket}/{json_upload_path}"
                        )
                    else:
                        new_ng_link = (
                            f"https://neuroglancer-demo.appspot.com/#!"
                            f"s3://{upload_bucket}/{json_upload_path.rstrip('/')}/{Path(output_json_file).name}"
                        )
